...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if (
            args.loss_type in ["hinge", "ipo", "bco_pair", "sppo_hard", "nca_pair", "apo_zero", "apo_down","ddpo","gdpo"]
    ...
    def dpo_loss(
        ...
        if self.loss_type == "sigmoid":
            ...
            q=self.c1/toch.clamp(self.c2-ref_logratios,self.c1/self.beta)  # Avoid division by zero
            # DDPO loss: https://huggingface.co/papers/2401.09966
            losses = (
                -F.logsigmoid(q * logits) * (1 - self.label_smoothing)
                + F.logsigmoid(q * logits) * self.label_smoothing
            ) / (1 - 2 * self.label_smoothing)
        elif self.loss_type == "gdpo":
            # GDPO loss: https://huggingface.co/papers/2401.09966
            losses = (
                (self.beta*(logits - ref_logratios)-self.c1)**2
            )
        elif self.loss_type == "tpo ":
            # TPO loss: https://huggingface.co/papers/2401.09966
            q=self.c1/toch.clamp(self.c2-ref_logratios,self.c1/self.beta)
            losses = (
                (q*(logits - ref_logratios)-self.c1)**2
            )
